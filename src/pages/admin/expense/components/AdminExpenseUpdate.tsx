...
import { useGetExpenseByIdQuery, useGetExpenseQuery } from "../../../../app/services/common/expense/expense";
...
const AdminCategoryUpdate = forwardRef<TAdminCategoryUpdateMethod>(
  (props, ref) => {
    ...
    // Api
    const categoryByIdRes = useGetExpenseByIdQuery({
      expenseId: open.updateableId ? open.updateableId.toString() : 0,
    });
    ...
    const handleSubmit = async ({ name, parent_id }: ICategoryAdd) => {
      if(!open.updateableId) return toast.warning('Bu harajat turi id topilmadi');
