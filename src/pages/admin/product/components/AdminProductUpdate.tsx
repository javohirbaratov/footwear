...
const AdminProductUpdate = forwardRef<TDrawerUpdateDataMethods>(

  (props, ref) => {
    // State
    const [open, setOpen] = useState<TDrawerUpdateDataState>({
      open: false,
      updateableId: null,
    });
    ...
    // Api
    const productByIdRes = useGetAdminProductByIdQuery({
      productId: open.updateableId ? open.updateableId.toString() : null,
    });
    const categoryRes = useGetCategoryQuery();
    const [addData, { isLoading }] = useAddAdminProductMutation();

    useEffect(() => {
      if (productByIdRes.data?.success && productByIdRes.data.data) {
        const { name, volume, article, price, block, desc, categories_list } =
          productByIdRes.data.data;

        formik.setFieldValue("name", name);
        formik.setFieldValue("categories_list", categories_list.map(item => item.id));
        formik.setFieldValue("volume", volume);
        formik.setFieldValue("article", article);
        formik.setFieldValue("price", price);
        formik.setFieldValue("block", block);
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                      {selected.map((value) => (
                        <Chip
                          key={value}
                          label={
                            categoryOption.find((item) => item.id === value)
                              ?.name
                          }
                        />
                      ))}
